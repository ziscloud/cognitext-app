...
export function InsertImageUploadedDialogBody({
    ...
    const loadImage = async (files: string | null) => {
        if (!files) {
            return;
        }
        console.log('settings', settings)
        const imageFileBaseName = await basename(files);
        if (settings.image.action === 1) {
            if (settings.image.preferRelativeFolder) {
                ...
                const toPath = await join(toDir, imageFileBaseName)
                await copyFile(files, toPath);
                setSrc(await join(dirName, imageFileBaseName))
            } else {
                if (settings.image.globalDir) {
                    ...
                    const toPath = await join(settings.image.globalDir, imageFileBaseName);
                    await copyFile(files, toPath);
                    setSrc(toPath)
                } else {
                    setSrc(files);
